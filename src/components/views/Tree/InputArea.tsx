...
const InputArea = () => {
	...
	const [newSectionButtonDisabled, setNewSectionButtonDisabled] =
		useState(true);
	const instructDaemonConfig = useAppSelector(
		(state) => state.daemon.instructDaemon,
	);
	const [instructDaemon, setInstructDaemon] = useState<InstructDaemon>(
		new InstructDaemon(instructDaemonConfig),
	);
	...
	const instructModel = useAppSelector(
		(state) => state.config.openAI.chatModel,
	);
	...
	useEffect(() => {
		setNewSectionButtonDisabled(
			activeIdeaIds.length === 0 || isCreatingSection === true,
		);
