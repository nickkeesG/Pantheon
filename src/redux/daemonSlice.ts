...
const daemonSlice = createSlice({
	...
	reducers: {
		...
		removeChatDaemon(state, action: PayloadAction<number>) {
			state.chatDaemons = state.chatDaemons.filter(
				(daemon) => daemon.id !== action.payload,
			);
		},
		updateChatDaemon(state, action: PayloadAction<ChatDaemonConfig>) {
			const index = state.chatDaemons.findIndex(
				(daemon) => daemon.id === action.payload.id,
			);
			if (index !== -1) {
		...
		replaceSlice: (_, action: PayloadAction<DaemonState>) => action.payload,
...
export const {
	...
	updateChatDaemon,
	updateBaseDaemon,
	replaceSlice: replaceDaemonSlice,
	resetSlice: resetDaemonSlice,
} = daemonSlice.actions;
