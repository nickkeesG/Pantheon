...
export type V3StoreState = { [P in keyof Omit<RootState, 'idea'>]: RootState[P]; } & {
  idea: V3IdeaState;
} & PersistedState;
...
export const V3to4Migration = (state: PersistedState): RootState => {
  if (state && 'idea' in state) {
    ...
    const newIdeas: Idea[] = Object.values(ideaState.ideas).map(({ textTokens, tokenSurprisals, ...idea }) => ({
      ...idea
    }));
